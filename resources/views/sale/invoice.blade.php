...
<div style="max-width:400px;margin:0 auto">
    ...
    <div class="hidden-print">
        <table>
            <tr>
                ...
                <td>
                    <button onclick="window.print();" class="btn btn-primary"><i class="dripicons-print"></i> {{trans('file.Print')}}
                    </button>
                </td>
            </tr>
        </table>
    ...
    <div id="receipt-data">
        ...
        <table class="table-data">
            <tbody>
            <?php use App\ProductBatch;use App\Product;use App\Variant;$total_product_tax = 0;?>
            @foreach($lims_product_sale_data as $key => $product_sale_data)
                ...
                <tr>
                    <td colspan="2">
                        ...
                        x {{number_format((float)($product_sale_data->total / $product_sale_data->qty))}}
                    ...
                    <td style="text-align:right;vertical-align:bottom">{{number_format((float)$product_sale_data->total)}}</td>
                </tr>
            @endforeach
            ...
            <tr>
                ...
                <th style="text-align:right">{{number_format((float)$lims_sale_data->total_price)}}</th>
            </tr>
            @if($general_setting->invoice_format == 'gst' && $general_setting->state == 1)
                <tr>
                    <td colspan="2">IGST</td>
                    <td style="text-align:right">{{number_format((float)$total_product_tax)}}</td>
                </tr>
            ...
            @if($lims_sale_data->order_tax)
                <tr>
                    ...
                    <th style="text-align:right">{{number_format((float)$lims_sale_data->order_tax)}}</th>
                </tr>
            @endif
            @if($lims_sale_data->order_discount)
                <tr>
                    <th colspan="2" style="text-align:left">{{trans('file.Order Discount')}}</th>
                    <th style="text-align:right">{{number_format((float)$lims_sale_data->order_discount)}}</th>
                </tr>
            @endif
            @if($lims_sale_data->coupon_discount)
                <tr>
                    <th colspan="2" style="text-align:left">{{trans('file.Coupon Discount')}}</th>
                    <th style="text-align:right">{{number_format((float)$lims_sale_data->coupon_discount)}}</th>
                </tr>
            @endif
            @if($lims_sale_data->shipping_cost)
                <tr>
                    <th colspan="2" style="text-align:left">{{trans('file.Shipping Cost')}}</th>
                    <th style="text-align:right">{{number_format((float)$lims_sale_data->shipping_cost)}}</th>
                </tr>
            @endif
            <tr>
            ...
            <tr>
                ...
                @else
                    <th class="centered" colspan="3">{{trans('file.In Words')}}: <span>{{str_replace("-"," ",$numberInWords)}}</span>
                        <span>{{$currency->code}}</span></th>
                @endif
            </tr>
            </tbody>
            <!-- </tfoot> -->
            ...
            @foreach($lims_payment_data as $payment_data)
                <tr style="background-color:#ddd;">
                    ...
                    <td style="padding: 5px;width:40%">{{trans('file.Amount')}}
                        : {{number_format((float)$payment_data->amount)}}</td>
                    <td style="padding: 5px;width:30%">{{trans('file.Change')}}
                        : {{number_format((float)$payment_data->change)}}</td>
                </tr>
            @endforeach
            <tr>
